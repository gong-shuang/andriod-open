...
import com.gs.imsdk.business.CommonCallback;
...
public class SessionAtPresenter extends BaseFragmentPresenter<ISessionAtView> {
    public static final String TAG = "SessionAtPresenter";

    public int mConversationType ;   //个人，群
    ...
    public SessionAtPresenter(BaseFragmentActivity context, String sessionId, int conversationType) {
        super(context);
...
//        });
    ...
    public void sendMessageText(){
        String msg = getView().getEtContent().getText().toString();
        getView().getEtContent().setText("");
        ...
        CommonInfo commonInfo = new CommonInfo(CommonInfo.TYPE_TEXT, msg,"","", mSessionId,
                Account.getUserId(), CommonInfo.STATE_SENDING);

        CommonPacket.getInstance().send( commonInfo,
                new CommonCallback(){

                    @Override
                    public void updateState(int state) {
                        if(state == CommonInfo.STATE_SENT){
                            Log.e(TAG, "send success!");

                        }else if(state == CommonInfo.STATE_SENT_RECEIVED){
                    ...
                    public void sendFailure() {
                        Log.e(TAG, "message is failure!");
                    }

                    @Override
                    public void receiveSucceed(CommonInfo commonInfo) {
                        Log.e(TAG, "receive message = " + commonInfo.getContent());
