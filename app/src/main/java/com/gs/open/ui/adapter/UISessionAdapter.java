package com.gs.open.ui.adapter;
...
public class UISessionAdapter  extends LQRAdapterForRecyclerView<Message> {
    ...
    private UiSessionAtPresenter mPresenter;
    ...
    public UISessionAdapter(Context context, List<Message> data, UiSessionAtPresenter presenter) {
        super(context, data);
        mContext = context;
        mData = data;
...
//      !(item.getContent() instanceof GroupNotificationMessage) &&
//               !(item.getContent() instanceof RecallNotificationMessage) &&
    ...
    private void setView(LQRViewHolderForRecyclerView helper, Message item, int position) {
        //根据消息类型设置消息显示内容
        int msgType = item.getType();
        LogUtils.e("msgType : " + msgType);
        if (msgType == Message.TYPE_STR) {
            MoonUtils.identifyFaceExpression(mContext, helper.getView(R.id.tvText), item.getContent(), ImageSpan.ALIGN_BOTTOM);
        }
        else if (msgType == Message.TYPE_PIC) {
            BubbleImageView bivPic = helper.getView(R.id.bivPic);
            String localPath = item.getLocalPath();
            if(localPath == null){
                //没有本地数据，此时显示缩略图
                Glide.with(mContext).load(R.mipmap.ic_huanlin).override(UIUtils.dip2Px(80), UIUtils.dip2Px(150)).centerCrop().into(bivPic);

                //下载本地图片，再保存到数据库中，使用异步完成
                MessageHelper.getLocalFileByOssAndUpdateDB(item);
            }else{
                //有本地数据，显示本地数据
                Glide.with(mContext).load(localPath).error(R.mipmap.default_img_failed).override(UIUtils.dip2Px(80), UIUtils.dip2Px(150)).centerCrop().into(bivPic);
            }
        } else if (msgType == Message.TYPE_AUDIO) {
            int timeLength = Integer.parseInt(item.getAttach());
            int increment = (int) (UIUtils.getDisplayWidth() / 2 / AppConst.DEFAULT_MAX_AUDIO_RECORD_TIME_SECOND * timeLength);

            RelativeLayout rlAudio = helper.setText(R.id.tvDuration, timeLength + "''").getView(R.id.rlAudio);
...
//        else if (msgType == Message.TYPE_FILE) {
//            String fileName = item.getLocalPath();
...
//                }
//            }
//        } else if (msgType == Message.TYPE_LOCATION) {
...
//        );
        ...
        helper.getView(R.id.ivAvatar).setOnClickListener(v -> {
            ...
            intent.putExtra("userInfo", item.getSender().getId());
 ...
 //       if (msgContent instanceof TextMessage || msgContent instanceof LocationMessage || msgContent instanceof VoiceMessage) {
        if(msgType == Message.TYPE_STR || msgType == Message.TYPE_LOCATION || msgType == Message.TYPE_VIDEO){
            //只需要设置自己发送的状态
    //        com.gs.open.temp.Message.SentStatus sentStatus = item.getSentStatus();
            int status = item.getStatus();
            if ( status == Message.STATUS_CREATED){
//            if (sentStatus == com.gs.open.temp.Message.SentStatus.SENDING) {
                helper.setViewVisibility(R.id.pbSending, View.VISIBLE).setViewVisibility(R.id.llError, View.GONE);
//            } else if (sentStatus == com.gs.open.temp.Message.SentStatus.FAILED) {
            } else if ( status == Message.STATUS_FAILED) {
                helper.setViewVisibility(R.id.pbSending, View.GONE).setViewVisibility(R.id.llError, View.VISIBLE);
//            } else if (sentStatus == com.gs.open.temp.Message.SentStatus.SENT) {
            } else if ( status == Message.STATUS_DONE ) {
...
//            BubbleImageView bivPic = helper.getView(R.id.bivPic);
//            boolean isSend = item.getMessageDirection() == com.gs.open.temp.Message.MessageDirection.SEND ? true : false;
//            if (isSend) {
//                com.gs.open.temp.Message.SentStatus sentStatus = item.getSentStatus();
//                if (sentStatus == com.gs.open.temp.Message.SentStatus.SENDING) {
...
//                if (isSend) {
//                    com.gs.open.temp.Message.SentStatus sentStatus = item.getSentStatus();
//                    if (sentStatus == com.gs.open.temp.Message.SentStatus.SENDING || fileMessage.getLocalPath() == null || (fileMessage.getLocalPath() != null && !new File(fileMessage.getLocalPath().getPath()).exists())) {
//                        if (!TextUtils.isEmpty(item.getExtra())) {
...
//                    }
//                } else {
//                    com.gs.open.temp.Message.ReceivedStatus receivedStatus = item.getReceivedStatus();
...
//        boolean isSend = item.getMessageDirection() == com.gs.open.temp.Message.MessageDirection.SEND ? true : false;
        long msgTime = item.getCreateAt().getTime();
        if (position > 0) {
            Message preMsg = mData.get(position - 1);
            long preMsgTime = preMsg.getCreateAt().getTime();
    ...
    /**
     * 在父类的 onCreateViewHolder 中已经设置了view，其第三个参数ViewType就是这个函数返回的。
     * 这是为了实现多种类型的 ViewType。
     * @param position
     * @return
     */
    @Override
