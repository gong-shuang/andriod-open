...
public class DBManager {
    ...
    private LinkedHashMap<String, User> mUserInfoCache;
    private List<Groups> mGroupsList;  // 只是临时使用，不用保存
...
//        deleteGroupMembers();
    ...
    public synchronized void saveOrUpdateFriend(Friend friend) {
        if (friend != null) {
            ...
            if(!friend.saveOrUpdate("userid = ?", friend.getUserId())){
                LogUtils.d(" save DB fail, friend.id:" + friend.getUserId());
            }
...
//        return DataSupport.where("userid != ?", UserCache.getId()).find(Friend.class);
        return DataSupport.where("userid != ?", "").find(Friend.class);
    }

    public synchronized void saveOrUpdateFriendsByFriends(List<Friend> friends) {
        if (friends != null && friends.size() > 0){
            for(Friend friend : friends){
                saveOrUpdateFriend(friend);
            }
        }
    }
    ...
    // gs-add
    public synchronized void saveGroupsAll(List<Groups> groupsList){
        if(groupsList != null && groupsList.size() != 0) {
            DataSupport.saveAll(groupsList);
        }
    }
...
//            BroadcastManager.getInstance(UIUtils.getContext()).sendBroadcast(AppConst.UPDATE_CONVERSATIONS);
     ...
     */
    public String getPortraitUri(User userInfo) {
        if (userInfo != null) {
            if (userInfo.getPortrait() != null) {
                    if (userInfo.getName() != null) {
                        return RongGenerate.generateDefaultAvatar(userInfo.getName(), userInfo.getId());
                    } else {
                        return null;
                    }
