...
import com.gs.factory.model.db.AppDatabase;
...
public class DbHelper {
     ...
     */
    private final Map<Class<?>, Set<ChangedListener>> changedListeners = new HashMap<>();   // 使用了观察者模型。
    ...
    private <Model extends BaseModel> Set<ChangedListener> getListeners(Class<Model> modelClass) {
        if (changedListeners.containsKey(modelClass)) {
            return changedListeners.get(modelClass);   // 返回的是一个集合。
        }
        return null;
    }
