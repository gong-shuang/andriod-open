...
import com.gs.factory.model.db.BaseDbModel;
import com.gs.factory.model.db.Session;
import com.gs.factory.utils.CollectionUtil;
...
public abstract class BaseDbRepository<Data extends BaseDbModel<Data>> implements DbDataSource<Data>,
    ...
    // 和Presenter交互的回调
    private SucceedCallback<List<Data>> callback;  // 数据加载成功的回调，在数据加载（子类的load()函数）之前设置该值。
    protected final LinkedList<Data> dataList = new LinkedList<>(); // 当前缓存的数据
    private Class<Data> dataClass; // 当前范型对应的真实的Class信息，这个值由泛型<Data>决定

    @SuppressWarnings("unchecked")
    public BaseDbRepository() {
        // 拿当前类的范型数组信息
        Type[] types = Reflector.getActualTypeArguments(BaseDbRepository.class, this.getClass());
        // 当子类是 SessionRepository 的时候，其 dataClass 的值为：com.gs.factory.model.db.Session
        dataClass = (Class<Data>) types[0];
    }

    @Override
    public void load(SucceedCallback<List<Data>> callback) {
        this.callback = callback;   // 这个是数据加载成功的回调
        // 进行数据库监听操作
        registerDbChangedListener();   // 这个是数据库的监听操作
    }
    ...
    // 数据库的监听操作，在加载数据之前，现要设置对数据库操作成功的监听
    // 数据库统一通知的地方：增加／更改
    @Override
